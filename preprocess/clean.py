"""
RUN ON SOFTLAYER INSTANCE TO PROCESS DATA (ZIP FILES) FROM SWIFT OBJECT STORE
"""
import os
import sys
import zipfile
  ...
  if zip_p.match(sender_folder):
    ...
    with contextlib.closing(zipfile.ZipFile(StringIO.StringIO(zip_file_obj[1]))) as z:
      ...
      for textfile in namelist:
        #report every 100 text files of progress
        count_tf +=1
        if count_tf%100 == 0 or count_tf>=num_tf:
          ...
          ### END FIX ###
            ...
            with open(filename,"w") as pf:
              ...
              for word in wordlist:
                ### MAIN CLEANING SCRIPT SHOULD BE APPLIED HERE ###

                ### ENDING MAIN CLEANING SCRIPT ###
                pf.write(word+"\n")
